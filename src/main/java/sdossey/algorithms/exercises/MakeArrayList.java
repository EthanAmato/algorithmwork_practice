...
public class MakeArrayList<E> extends AbstractList<E> {
	...
	public E get(int index) {
		...
		/*
		 * This will just return an item out of storage in most cases, but you must
		 * throw the IndexOutOfBoundsException if a bad index is passed in.
		 * 
	...
	public E set(int index, E value) {
		...
		/*
		 * YOu need to check and throw IndexOutOfBoundsException in case of a bad index
		 * just like with get.
		 */
		/*
		 * Be sure to return the old value previously at the index at the end of this
		 * method
		 */
	...
	/*
	 * This is so trivial I'm giving it to you. No code modification is allowed to
	 * this method, but make sure you update the size field when you add and remove
	 * items.
	 */
	@Override
	public int size() {
...
//    	System.out.println(index);
		 ...
		 * different than for get() and set() because it is legal specify the slot one
		 * past the end of the list in order to insert at the end of list. Throw an
		 * IndexOutOfBoundsException if the index is out of range.
		 * 
		 * 2. Ensure you have room in storage for the new element by comparing size and
		 * storage.length. If you don't have enough space.....
		 * 
		 * a) Allocate a new array of Objects that is twice the size of storage.length.
		 * b) copy items 0-size into the new array of Objects. c) set storage =
		 * newStorage; These steps will double the space available. We could choose to
		 * not waste space like this, but we'd end up copying so much we'd harm the
		 * efficiency of the implementation.
		 * 
		 * 3. Shift all items located at or past index right 1 spot in the array. When
		 * you shift an item right, you need to not overwrite the next item before it is
		 * shifted right, so you will have to proceed back to front for the shift.
		 * 
		...
		try {
			// Check to see if passed in index is the size of the array or less, otherwise
			// invalid
			if (index < 0 || index > size)
				throw new IndexOutOfBoundsException();
//
...
//
//			System.out.println("storage length: " + storage.length);
			...
			// Moving items from index over by one:
	...
	public E remove(int index) {
		 ...
		 * You will proceed as follows:
		 *
		 * 1. Check if the index is in range. Throw an IndexOutOfBoundsException if the
		 * index is out of range.
		 * 
		 * 2. Grab and store the old item at index.
		 * 
		 * 3. Shift all items past index left 1 spot in the array, replacing the item at
		 * index.
		 * 
		 ...
		 * shrinks we never shrink storage. Add code that if the storage.length becomes
		 * 4x "size" to reallocate and copy storage to a new storage half as big.
		 */
		try {
			if (index < 0 || index > size)
				throw new IndexOutOfBoundsException();
...
//		MakeArrayList<Integer> test = new MakeArrayList<>();
//		
//		test.add(16);
//
//		test.add(161);
